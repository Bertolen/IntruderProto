...
void AGuardCharacter::BeginPlay()
{
	...
	if (PawnSensingComp)
	{
		PawnSensingComp->OnSeePawn.AddDynamic(this, &AGuardCharacter::OnSeePlayer);
		PawnSensingComp->OnHearNoise.AddDynamic(this, &AGuardCharacter::OnHearNoise);
	}
	/*
	if (MeleeCollisionComp)
	{
		MeleeCollisionComp->OnComponentBeginOverlap.AddDynamic(this, &AGuardCharacter::OnMeleeCompBeginOverlap);
	}
	*/
...
void AGuardCharacter::Tick(float DeltaSeconds)
{
	...
	{
		AGuardController* AIController = Cast<AGuardController>(GetController());
		if (AIController)
		{
			bSensedTarget = false;
			/* Reset */
...
void AGuardCharacter::OnSeePlayer(APawn* Pawn)
{
	...
	*/
	...
	AGuardController* AIController = Cast<AGuardController>(GetController());
	AIntruderProtoCharacter* SensedPawn = Cast<AIntruderProtoCharacter>(Pawn);
